...
<!--begin::Quick Actions Panel-->
    ...
    $('._remove').on('click', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `${baseUrl}/${id}`, // Use baseUrl for the correct path
                    type: 'DELETE',
                    data: {
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="logModalLabel">Administration Logs</h5>
    ...
    function showLogs(category) {
        $('#logModal').modal('show');
        $('#logModalBody').html('<div class="text-center"><div class="spinner-border" role="status"><span class="sr-only">Loading...</span></div></div>');
        
        $.ajax({
            url: "{{ route('administration_logs.get_logs') }}",
            type: "GET",
            ...
            error: function(xhr) {
                if(xhr.status == 403) {
                     $('#logModalBody').html('<div class="alert alert-danger">Unauthorized: You do not have permission to view logs.</div>');
                } else {
                     $('#logModalBody').html('<div class="alert alert-danger">Error loading logs.</div>');
                }
            }
        });
    }
